...
# Modified version of dipy.shore
...
class BrainSuiteShoreModel(Cache):
    ...
    def fit(self, data):
        ...
        if self.regularization == "FEDI" :
            ...
            # Modifid by Haykel
            # Goal : minimize M*C - data

            Weighted_Loss_L2 =  cvxpy.sum_squares(weights @ (design_matrix - data))

            objective = cvxpy.Minimize(
                Weighted_Loss_L2 +
                self.lambdaN * cvxpy.quad_form(c, self.Nshore) +
                self.lambdaL * cvxpy.quad_form(c, self.Lshore)
            )
